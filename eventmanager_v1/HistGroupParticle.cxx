...
void HistGroupEvent::fill(const std::vector<fastjet::PseudoJet>& rJet)
{ 
  std::vecto<fastjet::pseudojet>::iterator fPart(rJet.begin());
  std::vecto<fastjet::pseudojet>::iterator lPart(rJet.end());
  for ( ; fPart != lPart; ++fPart )
    {
      h_eta->Fill(fPart->pseudorapidity());
      h_phi->Fill(fPart->psi_std());
